                  ...
                  {[...friends].sort((a, b) => (isOnline(b.last_login) ? 1 : 0) - (isOnline(a.last_login) ? 1 : 0)).map(f => {
                    ...
                    return (
                      <div key={f.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 10, background: 'rgba(255,255,255,.04)', border: '1px solid rgba(217,182,90,.1)' }}>
                        ...
                        {f.activeGame && (
                          <Link href={`/${f.activeGame.game}?join=${f.activeGame.code}`} onClick={() => setOpen(false)} style={{ padding: '5px 12px', borderRadius: 6, background: 'rgba(58,208,122,.15)', border: '1px solid rgba(58,208,122,.35)', color: '#3ad07a', fontSize: 11, fontFamily: 'var(--fs-head)', fontWeight: 700, letterSpacing: '.06em', textDecoration: 'none', textTransform: 'uppercase', flexShrink: 0 }}>
                            Join
                          </Link>
                        )}
                      </div>
